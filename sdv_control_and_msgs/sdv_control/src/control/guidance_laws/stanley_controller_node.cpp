...
class StanleyControllerNode : public rclcpp::Node
{
            ...
            if(path_arrived_) {
                if(waypoint_ < smooth_path_.poses.size()){
                    ...
                    if(waypoint_base_ == -1){
                        ...
                        p1_.position.z = transform.transform.translation.z - 1.8;
                    } else {
                    ...
                    current_ref_.poses[0].pose = p1_;
                    current_ref_.poses[1].pose = p2_;
                    ...
                    car_steering_pub_->publish(delta_);
    ...
    public:
        StanleyControllerNode() : Node("stanley_controller_node")
        {
            ...
            /* Publishers */
            car_steering_pub_ = this->create_publisher<std_msgs::msg::Float64>("/sdv/steering/delta_setpoint", 1);
            ...
            path_to_follow_ = this->create_subscription<nav_msgs::msg::Path>("/sdv/guidance/reference_path",
            ...
            current_ref_.header.frame_id = "odom";
